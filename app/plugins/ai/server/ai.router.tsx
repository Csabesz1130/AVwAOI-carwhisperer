...
export const AiRouter = Trpc.createRouter({
  ...
  explainTechnology: Trpc.procedure
    .input(
      z.object({
        technologyName: z.string(),
        aspect: z.enum(['howItWorks', 'benefits', 'drawbacks']).optional(),
      }),
    )
    .mutation(async ({ input }) => {
      try {
        ...
        const { technologyName, aspect } = input
        
        const basePrompt = `Explain the ${technologyName} car technology`
        const aspectPrompt = aspect 
          ? aspect === 'howItWorks'
            ? ` focusing on how it works`
            : ` focusing on its ${aspect}`
          : ''
        
        const answer = await AiService.generateText({
          prompt: basePrompt + aspectPrompt,
        })
        ...
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: error.message || 'Failed to generate technology explanation',
        })
      }
    }),
})
